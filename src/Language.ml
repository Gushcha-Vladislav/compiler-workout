...
module Expr =
  struct
    ...
    *)
    let eval _ _ = failwith "Not yet implemented"	    let toBool b = b <> 0
    ...
    let rec eval env ((st, input, output, r) as conf) expr =
      match expr with
      | Const c -> (st, input, output, Some c)
      | Var v -> (st, input, output, Some (State.eval st v))
      | Binop (op, x, y) ->
            let (st, input, output, Some r1) = eval env conf x in
            let (st, input, output, Some r2) = eval env (st, input, output, None) y in
            (st, input, output, Some action op r1 r2)
